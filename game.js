...
`;
const robeImg = new Image();
let robeLoaded = false;
robeImg.onload = () => { robeLoaded = true; };
robeImg.src = 'data:image/svg+xml;base64,' + btoa(robeSVG);

const spiderImg = new Image();
let spiderLoaded = false;
spiderImg.onload = () => { spiderLoaded = true; };
spiderImg.src = 'c64_spider_final.png';

const ghoulImg = new Image();
let ghoulLoaded = false;
ghoulImg.onload = () => { ghoulLoaded = true; };
ghoulImg.src = 'c64_ghoul_final.png';
...
const caveSlimeImg = new Image();
let caveSlimeLoaded = false;
caveSlimeImg.onload = () => { caveSlimeLoaded = true; };
caveSlimeImg.src = "cave_slime.png";

const giantRatImg = new Image();
let giantRatLoaded = false;
giantRatImg.onload = () => { giantRatLoaded = true; };
giantRatImg.src = "giant_rat.png";

const mimicChestImg = new Image();
let mimicChestLoaded = false;
mimicChestImg.onload = () => { mimicChestLoaded = true; };
mimicChestImg.src = "mimic_chest.png";

const restlessZombieImg = new Image();
let restlessZombieLoaded = false;
restlessZombieImg.onload = () => { restlessZombieLoaded = true; };
restlessZombieImg.src = "restless_zombie.png";

const blindWardenImg = new Image();
let blindWardenLoaded = false;
blindWardenImg.onload = () => { blindWardenLoaded = true; };
blindWardenImg.src = "blind_warden.png";

const ghostlyScribeImg = new Image();
let ghostlyScribeLoaded = false;
ghostlyScribeImg.onload = () => { ghostlyScribeLoaded = true; };
ghostlyScribeImg.src = "ghostly_scribe.png";

const echoWraithImg = new Image();
let echoWraithLoaded = false;
echoWraithImg.onload = () => { echoWraithLoaded = true; };
echoWraithImg.src = "echo_wraith.png";

const minotaurImg = new Image();
let minotaurLoaded = false;
minotaurImg.onload = () => { minotaurLoaded = true; };
minotaurImg.src = "minotaur.png";

const abyssalKnightImg = new Image();
let abyssalKnightLoaded = false;
abyssalKnightImg.onload = () => { abyssalKnightLoaded = true; };
abyssalKnightImg.src = "abyssal_knight.png";

const deepDwellerImg = new Image();
let deepDwellerLoaded = false;
deepDwellerImg.onload = () => { deepDwellerLoaded = true; };
deepDwellerImg.src = "deep_dweller.png";

const undeadKingImg = new Image();
let undeadKingLoaded = false;
undeadKingImg.onload = () => { undeadKingLoaded = true; };
undeadKingImg.src = "undead_king.png";

const bloodstoneGolemImg = new Image();
let bloodstoneGolemLoaded = false;
bloodstoneGolemImg.onload = () => { bloodstoneGolemLoaded = true; };
bloodstoneGolemImg.src = "bloodstone_golem.png";

const slimeLordImg = new Image();
let slimeLordLoaded = false;
slimeLordImg.onload = () => { slimeLordLoaded = true; };
slimeLordImg.src = "slime_lord.png";

const vaultGuardianImg = new Image();
let vaultGuardianLoaded = false;
vaultGuardianImg.onload = () => { vaultGuardianLoaded = true; };
vaultGuardianImg.src = "vault_guardian.png";

const ironGiantImg = new Image();
let ironGiantLoaded = false;
ironGiantImg.onload = () => { ironGiantLoaded = true; };
ironGiantImg.src = "iron_giant.png";

const voidCultistImg = new Image();
let voidCultistLoaded = false;
voidCultistImg.onload = () => { voidCultistLoaded = true; };
voidCultistImg.src = "void_cultist.png";

const boneDragonImg = new Image();
let boneDragonLoaded = false;
boneDragonImg.onload = () => { boneDragonLoaded = true; };
boneDragonImg.src = "bone_dragon.png";

const whisperingTerrorImg = new Image();
let whisperingTerrorLoaded = false;
whisperingTerrorImg.onload = () => { whisperingTerrorLoaded = true; };
whisperingTerrorImg.src = "whispering_terror.png";

const depthCoreImg = new Image();
let depthCoreLoaded = false;
depthCoreImg.onload = () => { depthCoreLoaded = true; };
depthCoreImg.src = "depth_core.png";
...
// BESTIARY inlined from state.js
const BESTIARY = {
...
// Armor data inlined from state.js to avoid module import issues
const LEVEL_ARMOR = [
...
// Weapons data inlined from state.js
const LEVEL_WEAPONS = [
...
function generateMap(W, H) {
    ...
    state.enemies = [];
    state.items = [];
    state.torches = []; // Reset torches each map generation
    let spawned = 0;
        ...
        for (let attempts = 0; attempts < 100; attempts++) {
            ...
            if (map[y][x] === 0 && (x !== state.player.x || y !== state.player.y)) {
                if (!state.enemies.find(e => e.x === x && e.y === y)) {
                    ...
                    // Low-level enemies in passageways; floor-specific monsters always in open rooms.
                    ...
                    // Fallback: after 80 attempts let low-level enemies go anywhere, but
                    // never place high-level enemies in corridors on any level
                    if (attempts > 80 && !isHighLevel) validPlacement = true;

                        if (validPlacement) {
                            // Use BESTIARY HP if available for this floor, else scale with lvl
                            const bestiaryFloor = BESTIARY[state.level];
                            const monsterDef = bestiaryFloor?.find(m => m.name === type);
                            let hp = monsterDef
                                ? Math.ceil(monsterDef.hp * (0.9 + lvl * 0.1)) // slight scaling
                                : 10 + (lvl * 4);
    ...
    // Per-level enemy spawn pools — one entry per dungeon floor
    const levelEnemyPools = {
        1:  null, // handled separately — one skeleton per level 1-5
    ...
    if (state.level === 1) {
        // Level 1: one Skeleton at each level 1-5 (beginner’s dungeon — learn the mechanic)
        ...
        for (let i = 0; i < numEnemies; i++) {
            ...
            if (trySpawnEnemy(type, lvl)) {
                spawned++;
            }
        }
    }

    // Always ensure at least one level-5 enemy who holds the Gold Key
    // On level 1 the level-5 Skeleton was already spawned above — just give it the key
    if (state.level === 1) {
        ...
        // Gold key carrier: a strong enemy from the current floor's pool
        const floorPool = levelEnemyPools[state.level] || levelEnemyPools[3];
        const lvl5Pool = floorPool.filter(t => t !== 'Mimic Chest'); // mimics handled separately
    ...
    // Fallback Gold Key on floor if spawn failed
    if (!goldKeyDropped) {
        for (let attempts = 0; attempts < 100; attempts++) {
    ...
    // Extra spawns using the floor's own pool (scale with depth)
    if (state.level >= 2) {
        const floorPool = levelEnemyPools[state.level] || levelEnemyPools[3];
        ...
        for (let att = 0; att < 50 && extraSpawned < extraCount; att++) {
            ...
            if (type === 'Mimic Chest') continue; // don't duplicate mimics via extras
    ...
    // Potion stashes quest removed as requested
    ...
    for (let i = 0; i < validAnchors.length && roomsCarved < numLockedRooms; i++) {
        ...
        // Place items
        if (roomsCarved === 0) {
            // First room ALWAYS gets the level weapon
            ...
            // Second room ALWAYS gets the Black Key
            state.items.push({ x: a.centerRoomX, y: a.centerRoomY, type: 'Black Key', name: 'Black Key' });
            // On Level 3, also place the Torch in this room
            if (state.level === 3) {
    ...
    // floor potion spawning logic removed as per user request (they now only appear in secret walls)
    ...
    // Spawn 1 piece of Armor 50% of the time for the current level
    ...
    // Flood-fill from player start to find all reachable floor tiles
    ...
    // Only walls adjacent to a REACHABLE floor tile (player can always approach and reveal them)
    let wallCoords = [];
    for (let y = 1; y < H - 1; y++) {
        for (let x = 1; x < W - 1; x++) {
            if (map[y][x] === 1) {
    ...
    let targetSecrets = 9 + state.level; // Updated: 10 on L1, 11 on L2, etc. (Question 4 requested 10 on L1, +1 per sub level)
    ...
    // --- Generate torches for Level 3 ---
    if (state.level === 3) {
        for (let y = 1; y < H - 1; y++) {
            for (let x = 1; x < W - 1; x++) {
                if (map[y][x] === 1) {
                    // Only place torch on walls adjacent to open floor
...
function updateUIState() {
    ...
    // Attach unequip prompt to slot elements
...
window.equipItem = function (idx) {
    ...
    // Torch goes to a free hand slot
    if (item === 'Torch') {
        if (!state.hands.left) {
    ...
    for (let i = 0; i < state.enemies.length; i++) {
        ...
        if (e.x === tx && e.y === ty && e.state !== 'dead') {
            ...
            if (Math.random() < 0.2) {
                ...
                // Enemies take 20% less damage (rebalance)
                finalDmg = Math.ceil(finalDmg * 0.8);

                // User requested level 4 and 5 skeletons to take ~15% more damage (diff scaling)
                if (e.level >= 4) {
                ...
                // Show hit flash on enemy (red overlay)
                e.state = 'hit';
            ...
            if (e.hp > 0) {
                setTimeout(() => {
                    if (Math.random() < 0.2) {
                        ...
                        // Look up BESTIARY attack for this enemy type
                        const bEntry = Object.values(BESTIARY).flat().find(m => m.name === e.type);
                        if (bEntry) {
                            // Use BESTIARY attack with ±30% variance
                        ...
                        // All enemies 20% stronger - Attack
                        finalEnemyDmg = Math.ceil(finalEnemyDmg * 1.2);
                        
                        // Mitigate damage with armor defense, ensuring at least 1 damage is always taken if hit
...
state.map = generateMap(state.level === 1 ? 27 : 27, state.level === 1 ? 27 : 27); // Level 1 is 27x27 (30% larger)
...
function init() {
    ...
    function closeTutorial() {
        if (tutorialNoShow && tutorialNoShow.checked) {
            localStorage.setItem('tdd_tutorial_seen', 'true');
        }
...
function bindControls() {
    ...
    window.addEventListener('keydown', (e) => {
        // Tutorial dismissal takes priority
...
function handleMove(nx, ny) {
    ...
    // Clear lingering text only when actually moving (not when attacking a live enemy)
    if (!liveEnemyAhead) {
    ...
    if (cell === 0 || cell === 3) {
        ...
        if (!liveEnemyAhead) {
            ...
            checkAutoLoot();
            advanceTurn();
            // Reveal any disguised Mimic Chest within 2 tiles
            state.enemies.forEach(e => {
                if (e.type === 'Mimic Chest' && e.disguised && e.state !== 'dead') {
                    ...
                    if (dist <= 2) {
                        ...
                        playSound('hit');
                        // Ambush hit — mimic strikes before the player can react
...
function nextLevel() {
    ...
    let w = 27, h = 27; // Level 1: 27x27 (30% larger)
    if (state.level === 2) {
        w = 31; h = 31;
...
// Initialize offscreen canvas for complex destination-out masking
const lightingCanvas = document.createElement('canvas');
lightingCanvas.width = 400; // GAME_WIDTH
lightingCanvas.height = 300; // GAME_HEIGHT
...
function drawLighting() {
    ...
    const ambientDark = hasTorchEquipped ? 0.3 : 0.82; // Torch dramatically brightens the area
    ...
    // Player's personal light (bigger when torch held)
    ...
    for (let t of state.torches) {
        ...
        // Make sure it's somewhat in front
        if (tz >= 0 && tz <= MAX_DEPTH && Math.abs(tx) <= 6) {
            const p = project(tx, 0.4, tz);
            // Time offset by map coordinate for irregular blinking across different torches
            const flicker = 0.92 + Math.sin(Date.now() / 150 + tx * tz) * 0.08; 
            const rMask = (500 / (tz + 0.5)) * flicker; // Doubled radius for wider illumination
    ...
    // Draw the lighting mask onto main canvas
...
function drawTorch(pts, isSide) {
    // Find center area of the polygon to draw a glowing orb
    const cx = (pts[0].x + pts[1].x + pts[2].x + pts[3].x) / 4;
    // Place torch slightly above center center 
    const cy = (pts[0].y * 2 + pts[1].y * 2 + pts[2].y + pts[3].y) / 6; 
    
    // Scale by depth (height of polygon)
    const height = Math.abs(pts[0].y - pts[3].y);
    const r = height * 0.1;
    if (r < 1) return; // Too small
    
    // Flicker based on time and position so they don't all pulse together identically
    ...
    const drawFlame = (color, radius, yOffset, blur) => {
        ...
        // Base of flame is rounder, top is jagged and taller
        const points = 7;
        for (let i = 0; i < points * 2; i++) {
            // angle starts from bottom (Math.PI / 2)
            const angle = (i / (points * 2)) * Math.PI * 2 + Math.PI / 2; 
            let currentR = radius;
            
            if (i % 2 !== 0) {
                // Inner points for jaggedness
                currentR *= 0.6;
            }
            
            // Make upward points longer (flame tail)
    ...
    drawFlame(colors.white, r * 0.4 * flicker, r * 0.2, 0); // Core is smaller
}
...
function renderEnemy(x, z, enemy) {
    ...
    let h = pBot.y - pTop.y;
    let w = h * 0.5; // default aspect ratio 1:2
    
    // Adjust aspect ratio for specific enemy types
    const origH = h;
    if (enemy.type === 'Spider') {
        w = h * 1.37; // Spiders are wider
    } else if (enemy.type === 'Cave Slime') {
        w = origH * 1.5; // Wide and squat
        h = origH * 0.55;
    } else if (enemy.type === 'Mimic Chest') {
        w = h * 1.0; // Chest is roughly square
    }
    ...
    if (enemy.state === 'dead') {
        ...
        // Sit on the floor — push down by the height we removed, plus a gentle wobble
        yOff = (origH - h) + Math.sin(Date.now() / 200 + enemy.x * 2) * 2;
    } else if (!enemy.disguised) {
        // Floating/bobbing effect for non-disguised enemies
        yOff = Math.sin(Date.now() / 300 + enemy.x + enemy.y) * 5;
    }

    // Sprite lookup table — maps enemy type to { img, loaded, filter? }
    const SPRITE_MAP = {
    ...
    // Mimic disguise: use chest image while hidden, reveal on proximity
    ...
    if (isLoaded) {
        // Apply sprite filter (from SPRITE_MAP or hardcoded specials)
        ...
        // Draw the robe over Cloaked Skeleton
    ...
    // Draw Nameplate if alive (disguised mimics show no nameplate — they look like chests)
